package src.utils {
	...
	public class BrushPattern extends Sprite {
		...
		public function BrushPattern ( groundPoints:Vector.<Point>, $texture:BitmapData, $terrainContentTexture:BitmapData, $textureMaxWidth:Number, $textureHeight:Number ) {
			...
			map = renderPreProcess ( map, $texture.width, $textureMaxWidth );
			g = holderS.graphics;
			...
			for ( var j:int = 0; j < mapPieces.length; j++ ) {
				...
				_fillBack.graphics.beginBitmapFill ( _terrainContentTexture );
				for ( var i:uint = 0; i < tmp.length - 1; i++ ) {
					var tmp2:Array = tmp[i].split ( "<" );
					savedGroundPoints.push( new Point( tmp2[2], tmp2[3] ) );
					if ( j == 0 && i == 0 ) {
						_fillBack.graphics.moveTo ( Number ( tmp2[0] ), Number ( tmp2[1] ) )
						_fillBack.graphics.lineTo ( Number ( tmp2[0] ), Number ( tmp2[1] ) + 700 );
					} else {
						_fillBack.graphics.lineTo ( Number ( tmp2[2] ), Number ( tmp2[3] ) + 700 );
					}
					if ( i == 0 ) {
				...
				for ( i; i > 0; i-- )
				{
					_fillBack.graphics.lineTo( savedGroundPoints[i].x + $textureMaxWidth / 2, savedGroundPoints[i].y );
				}
		...
		private function render () :void {
			g.clear ();
			g.beginBitmapFill ( _terrainTexture, null, true, true );
